...
#include "robotnik_gazebo_ground_truth_odom/robotnik_gazebo_ground_truth_odom.h"

namespace gazebo
{
GZ_REGISTER_MODEL_PLUGIN(RobotnikGazeboGroundTruthOdom);

////////////////////////////////////////////////////////////////////////////////
// Constructor
RobotnikGazeboGroundTruthOdom::RobotnikGazeboGroundTruthOdom()
{
  this->seed = 0;
}
...
////////////////////////////////////////////////////////////////////////////////
// Load the controller
void RobotnikGazeboGroundTruthOdom::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!_sdf->HasElement("worldBodyName"))
  {
    ...
    this->world_body_name_ = _sdf->GetElement("worldBodyName")->Get<std::string>();
  ...
  if (!_sdf->HasElement("worldFrameName"))
  {
    ...
    this->world_frame_name_ = _sdf->GetElement("worldFrameName")->Get<std::string>();
  ...
  if (!_sdf->HasElement("broadcastTF"))
  {
    ...
    this->broadcastTF_ = _sdf->GetElement("broadcastTF")->Get<bool>();
  ...
  this->tf_frame_name_ = tf::resolve(prefix, this->world_frame_name_);
  this->transform_broadcaster_.reset(new tf::TransformBroadcaster());
  ...
  if (this->world_body_name_ != "/world" &&
      this->world_body_name_ != "world" &&
      this->world_body_name_ != "/map" &&
      this->world_body_name_ != "map")
  {
  ...
  this->update_connection_ = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&RobotnikGazeboGroundTruthOdom::UpdateChild, this));
}

////////////////////////////////////////////////////////////////////////////////
// Update the controller
void RobotnikGazeboGroundTruthOdom::UpdateChild()
{
  if (!this->link_)
    return;
  ...
  if (this->update_rate_ > 0 &&
    ...
    if (tmp_dt != 0)
    {
      ...
      if (this->topic_name_ != "")
      {
        ...
        this->pose_msg_.child_frame_id = this->frame_name_;
        ...
        //static ignition::math::Pose3d last_pose = pose;
        //ROS_INFO_STREAM("orig vpos: " << vpos.X() << ", " << vpos.Y());
        //vpos = last_pose.Rot().Inverse().RotateVector(vpos);
        //ROS_INFO_STREAM(last_pose.Rot());
        //ROS_INFO_STREAM("ko vpos: " << vpos.X() << ", " << vpos.Y());
        //static geometry_msgs::Pose last_odom_pose = this->pose_msg_.pose.pose;
        //vpos.X() = (this->pose_msg_.pose.pose.position.x - last_odom_pose.position.x) / tmp_dt;
        //vpos.Y() = (this->pose_msg_.pose.pose.position.y - last_odom_pose.position.y) / tmp_dt;
        //ROS_INFO_STREAM("bad vpos: " << vpos.X() << ", " << vpos.Y() << "\n");
        //float yaw = pose.Rot().Yaw();
        //double vx = cosf(yaw) * vpos.X() + sinf(yaw) * vpos.Y();
        //double vy = cosf(yaw) * vpos.Y() - sinf(yaw) * vpos.X(); 
        //ROS_INFO_STREAM("ok vpos: " << vx << ", " << vy << "\n");
        //last_odom_pose = this->pose_msg_.pose.pose;

        // velocity should be expressed in the local link.
        static ignition::math::Pose3d last_pose = pose;
        vpos = last_pose.Rot().Inverse().RotateVector(vpos);
        veul = last_pose.Rot().Inverse().RotateVector(veul);
        last_pose = pose;
        ...
        double gn2 = 0.1; //this->gaussian_noise_*this->gaussian_noise_;
        ...
        if (broadcastTF_ == true) {
          tf::Quaternion qt(pose.Rot().X(), pose.Rot().Y(), pose.Rot().Z(), pose.Rot().W());
          tf::Vector3    vt(pose.Pos().X(), pose.Pos().Y(), pose.Pos().Z());
          tf::Transform base_footprint_to_odom(qt, vt);
          transform_broadcaster_->sendTransform(
          tf::StampedTransform(base_footprint_to_odom, ros::Time::now(), this->pose_msg_.header.frame_id,
            this->pose_msg_.child_frame_id ));
        }
      }
...
double RobotnikGazeboGroundTruthOdom::GaussianKernel(double mu, double sigma)
{
...
void RobotnikGazeboGroundTruthOdom::QueueThread()
{
