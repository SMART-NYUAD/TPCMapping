...
#include <robotnik_force_based_move/robotnik_force_based_move.h>

namespace gazebo 
{

  RobotnikForceBasedMove::RobotnikForceBasedMove() {}

  RobotnikForceBasedMove::~RobotnikForceBasedMove() {}

  // Load the controller
  void RobotnikForceBasedMove::Load(physics::ModelPtr parent,
      sdf::ElementPtr sdf) 
  {
    ...
    command_watchdog_shown_ = false;
    command_watchdog_ = ros::Duration(2);
    if (!sdf->HasElement("commandWatchdog"))
    {
      ROS_WARN("PlanarMovePlugin (ns = %s) missing <commandWatchdog>, "
          "defaults to \"%f\"", 
          robot_namespace_.c_str(), command_watchdog_.toSec());
    }
    else
    {
      command_watchdog_ = ros::Duration(sdf->GetElement("commandWatchdog")->Get<double>());
    }
    ...
    ros::SubscribeOptions so =
      ros::SubscribeOptions::create<geometry_msgs::Twist>(command_topic_, 1,
          boost::bind(&RobotnikForceBasedMove::cmdVelCallback, this, _1),
          ros::VoidPtr(), &queue_);
    ...
    callback_queue_thread_ = 
      boost::thread(boost::bind(&RobotnikForceBasedMove::QueueThread, this));

    // listen to the update event (broadcast every simulation iteration)
    update_connection_begin_ = 
      event::Events::ConnectWorldUpdateBegin(
          boost::bind(&RobotnikForceBasedMove::UpdateChildBegin, this));

    update_connection_end_ =
        event::Events::ConnectWorldUpdateEnd(boost::bind(&RobotnikForceBasedMove::UpdateChildEnd, this));
  }
  ...
  void RobotnikForceBasedMove::UpdateChildEnd()
  {
    ...
    if (ros::Time::now() - command_last_stamp_ > command_watchdog_)
    {
      if (command_watchdog_shown_ == false)
      {
        ROS_WARN("PlanarMovePlugin (ns = %s): Robot stopped and pose forced to prevent slipping away.", 
          robot_namespace_.c_str());
        command_watchdog_shown_ = true;
      }
      parent_->SetLinearVel(ignition::math::Vector3d(0, 0, 0));
      parent_->SetAngularVel(ignition::math::Vector3d(0, 0, 0));
      parent_->SetWorldPose(last_valid_pose_);
    }
    else
    {
      command_watchdog_shown_ = false;
      last_valid_pose_ = parent_->WorldPose();
    }
  }
  
  // Initialize the controller
  void RobotnikForceBasedMove::InitChild() {
    last_valid_pose_ = parent_->WorldPose();
  }


  // Finalize the controller
  void RobotnikForceBasedMove::FiniChild() {
    alive_ = false;
    queue_.clear();
    queue_.disable();
    rosnode_->shutdown();
  ...
  void RobotnikForceBasedMove::cmdVelCallback(
      const geometry_msgs::Twist::ConstPtr& cmd_msg) 
  {
    ...
    command_last_stamp_ = ros::Time::now();
  }

  void RobotnikForceBasedMove::QueueThread()
  {
  ...
  void RobotnikForceBasedMove::publishOdometry(double step_time)
  {
  ...
  tf::Transform RobotnikForceBasedMove::getTransformForMotion(double linear_vel_x, double linear_vel_y, double angular_vel, double timeSeconds) const
  {
    tf::Transform tmp;
    tmp.setIdentity();
  ...
  GZ_REGISTER_MODEL_PLUGIN(RobotnikForceBasedMove)
}
